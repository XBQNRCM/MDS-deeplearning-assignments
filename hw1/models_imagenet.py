...
class BaselineCNN(nn.Module):
    """
    基线CNN模型 - 适配TinyImageNet (64x64, 200类)
    
    架构调整：
    - 输入: 64x64 -> 经过3次MaxPool2d(2) -> 8x8
    - 特征维度: 256 * 8 * 8 = 16384
    """
    def __init__(self, num_classes=200):
        super().__init__()
        self.features = nn.Sequential(
            nn.Conv2d(3, 128, 3, padding=1), nn.ReLU(inplace=True), nn.MaxPool2d(2), nn.Dropout(0.3),  # 32x32
            nn.Conv2d(128, 256, 3, padding=1), nn.ReLU(inplace=True), nn.MaxPool2d(2), nn.Dropout(0.3),  # 16x16
            nn.Conv2d(256, 512, 3, padding=1), nn.ReLU(inplace=True),
            nn.Conv2d(512, 512, 3, padding=1), nn.ReLU(inplace=True),
            nn.Conv2d(512, 256, 3, padding=1), nn.ReLU(inplace=True), nn.MaxPool2d(2), nn.Dropout(0.3),  # 8x8
        )
...
class CNNWithResidual(nn.Module):
    """
    带残差连接的CNN - 改进因子(a) - 适配TinyImageNet
    ...
    def __init__(self, num_classes=200):
        ...
        # 残差块 - 添加额外的块以处理更大的输入
        self.res_block1 = ResidualBlock(64, 128, stride=2)   # 32x32
        self.res_block2 = ResidualBlock(128, 256, stride=2)  # 16x16
        self.res_block3 = ResidualBlock(256, 512, stride=2)  # 8x8
        self.res_block4 = ResidualBlock(512, 512, stride=2)  # 4x4
...
class DeeperWiderCNN(nn.Module):
    """
    更深更宽的CNN - 改进因子(b) - 适配TinyImageNet
    
    架构调整：
    - 64x64 输入
    - 保持相同的深度和宽度，使用AdaptiveAvgPool2d自动适配
    """
    def __init__(self, num_classes=200):
        super().__init__()
        self.features = nn.Sequential(
            ...
            nn.MaxPool2d(2), nn.Dropout(0.2),  # 32x32
            
            # 第二层 - 32x32
            nn.Conv2d(128, 256, 3, padding=1), nn.BatchNorm2d(256), nn.ReLU(inplace=True),
            nn.Conv2d(256, 256, 3, padding=1), nn.BatchNorm2d(256), nn.ReLU(inplace=True),
            nn.MaxPool2d(2), nn.Dropout(0.2),  # 16x16
            
            # 第三层 - 16x16
            nn.Conv2d(256, 512, 3, padding=1), nn.BatchNorm2d(512), nn.ReLU(inplace=True),
            nn.Conv2d(512, 512, 3, padding=1), nn.BatchNorm2d(512), nn.ReLU(inplace=True),
            nn.MaxPool2d(2), nn.Dropout(0.2),  # 8x8
            ...
            nn.AdaptiveAvgPool2d((1, 1)),  # 使用自适应池化
        )
...
class CNNWithSEAttention(nn.Module):
    """
    带SE注意力的CNN - 改进因子(e) - 适配TinyImageNet
    ...
    def __init__(self, num_classes=200):
        ...
        # 分类器 - 使用自适应池化后的特征
        # 64x64 -> 32x32 -> 16x16 -> 8x8, 256 channels
        self.classifier = nn.Sequential(
            nn.Flatten(),
            nn.Linear(256 * 8 * 8, 512), nn.ReLU(inplace=True), nn.Dropout(0.5),
            nn.Linear(512, 256), nn.ReLU(inplace=True), nn.Dropout(0.5),
